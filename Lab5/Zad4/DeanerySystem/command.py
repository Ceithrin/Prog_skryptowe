...
class PrintCommand(Command):
    ...
    def __init__(self, rcv):  #podaję mu stworzonego receivera
        self.rcv = rcv

    def execute(self, tt):
        self.rcv.do_print(tt)  #wykonuję funkcję z klasy Receiver, w której jest zdefiniowana
...
class Invoker():
    ...
    def __init__(self, tt_dict):
        self.tt_dict = tt_dict #tworząc ten obiekt podaje mu istniejące timetables
        self.valid_cmd = ['p', 'd+', 'd-', 't+', 't-', 'sb+', 'sb-']  #mówię mu jakie są poprawne komendy

    def set_on_print(self, command):  #przypisuję metodom komendy
    ...
    def parse_exec(self, cmdstr): #tutaj zajmuję się poleceniami z linni komend
        cmdtab = cmdstr.split()
        if len(cmdtab) < 2:
        ...
        cmd = cmdtab[1]
        tt = self.tt_dict[cmdtab[0]]  #ustawiam ktorym timetable się zajmuję
        if cmd == 'p':
            if isinstance(self._on_print, Command): #sprawdź czy self._on_print jest typu COmmand
                self._on_print.execute(tt) #jeśli tak to ją wykonaj na podanej timetable
        elif cmd == 'd+':
...
class Receiver:  # w receiverze definiuję tak  na prawdę te komendy - ustalam, co konkretnie robią na tablicach
...
if __name__ == '__main__':
    ...
    inv = Invoker(tts)
    inv.set_on_print(PrintCommand(rcv))  #tutaj ustawiam invokerowi komendy klasy dziedziczącej z command
